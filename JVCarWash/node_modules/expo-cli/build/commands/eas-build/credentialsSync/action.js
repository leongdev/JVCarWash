...
async function credentialsSyncAction(projectDir, options) {
  ...
  _xdl().Analytics.logEvent(_types().AnalyticsEvent.CREDENTIALS_SYNC_COMMAND, trackingCtx);
...
async function updateRemoteCredentialsAsync(projectDir, platform, trackingCtx) {
  ...
  if ([_types().BuildCommandPlatform.ALL, _types().BuildCommandPlatform.ANDROID].includes(platform)) {
    try {
      ...
      _xdl().Analytics.logEvent(_types().AnalyticsEvent.CREDENTIALS_SYNC_UPDATE_REMOTE_SUCCESS, { ...trackingCtx,
        platform: 'android'
      });
    } catch (error) {
      _xdl().Analytics.logEvent(_types().AnalyticsEvent.CREDENTIALS_SYNC_UPDATE_REMOTE_FAIL, { ...trackingCtx,
        platform: 'android',
...
async function updateLocalCredentialsAsync(projectDir, platform, trackingCtx) {
  ...
  if ([_types().BuildCommandPlatform.ALL, _types().BuildCommandPlatform.ANDROID].includes(platform)) {
    try {
      ...
      _xdl().Analytics.logEvent(_types().AnalyticsEvent.CREDENTIALS_SYNC_UPDATE_LOCAL_SUCCESS, { ...trackingCtx,
        platform: 'android'
      });
    } catch (error) {
      _xdl().Analytics.logEvent(_types().AnalyticsEvent.CREDENTIALS_SYNC_UPDATE_LOCAL_FAIL, { ...trackingCtx,
        platform: 'android',
        reason: error.message
      });
