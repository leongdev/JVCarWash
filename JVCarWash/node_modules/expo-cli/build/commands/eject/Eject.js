...
async function ejectAsync(projectRoot, options) {
  ...
  if (await (0, _ProjectUtils().usesOldExpoUpdatesAsync)(projectRoot)) {
    _log().default.nested(`- 🚀 ${((0, _terminalLink().default)('expo-updates', 'https://github.com/expo/expo/blob/master/packages/expo-updates/README.md'), {
      fallback: text => text
    })} has been configured in your project. Before you do a release build, make sure you run ${_chalk().default.bold('expo publish')}. ${(0, _TerminalLink().learnMore)('https://expo.fyi/release-builds-with-expo-updates')}`);
  }
...
async function createNativeProjectsFromTemplateAsync(projectRoot) {
  ...
  exp.android = (_exp$android = exp.android) !== null && _exp$android !== void 0 ? _exp$android : {};
  ...
  exp.ios = (_exp$ios = exp.ios) !== null && _exp$ios !== void 0 ? _exp$ios : {};
  ...
  await _fsExtra().default.writeFile(_path().default.resolve('app.json'), JSON.stringify({
    expo: exp
  }, null, 2)); // TODO: if app.config.js, need to provide some other info here
...
async function promptForNativeAppNameAsync({
  name
}) {
  (0, _log().default)('First, we want to clarify what names we should use for your app:');
  const result = await (0, _prompt().default)([{
    name: 'name',
