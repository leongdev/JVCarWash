...
function _default(program) {
  ...
  program.command('client:install:ios').description('Install the Expo client for iOS on the simulator').helpGroup('client').asyncAction(async () => {
  ...
  program.command('client:install:android').description('Install the Expo client for Android on a connected device or emulator').helpGroup('client').asyncAction(async () => {
    const currentSdkConfig = await ClientUpgradeUtils().getExpoSdkConfig(process.cwd());
    const currentSdkVersion = currentSdkConfig ? currentSdkConfig.sdkVersion : undefined;
    ...
    if (currentSdk && recommendedClient) {
      ...
      if (answer.upgradeToLatest) {
        await _xdl().Android.upgradeExpoAsync(latestClient === null || latestClient === void 0 ? void 0 : latestClient.url);
        (0, _log().default)('Done!');
        return;
      }
    }
