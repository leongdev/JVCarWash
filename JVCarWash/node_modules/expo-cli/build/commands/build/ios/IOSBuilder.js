...
class IOSBuilder extends _BaseBuilder().default {
  ...
  async _setupDistCert(ctx, appLookupParams) {
    try {
      ...
      if (distCertFromParams) {
        ...
        await (0, _route().runCredentialsManager)(ctx, new (_SetupIosDist().SetupIosDist)(appLookupParams, nonInteractive));
      }
    } catch (e) {
  ...
  async _setupPushCert(ctx, appLookupParams) {
    try {
      ...
      if (pushKeyFromParams) {
        ...
        await (0, _route().runCredentialsManager)(ctx, new (_SetupIosPush().SetupIosPush)(appLookupParams, nonInteractive));
      }
    } catch (e) {
  ...
  async _setupProvisioningProfile(ctx, appLookupParams) {
    try {
      ...
      if (provisioningProfileFromParams) {
        ...
        await (0, _route().runCredentialsManager)(ctx, new (_SetupIosProvisioningProfile().SetupIosProvisioningProfile)(appLookupParams, nonInteractive));
      }
    } catch (e) {
  ...
  async clearCredentials(ctx, appLookupParams, credsToClear) {
    ...
    if (credsToClear.provisioningProfile && provisioningProfile) {
      const view = new (_IosProvisioningProfile().RemoveProvisioningProfile)(appLookupParams.accountName, shouldRevokeOnApple, nonInteractive);
    ...
    if (credsToClear.distributionCert && distributionCert) {
      const view = new (_IosDistCert().RemoveIosDist)(appLookupParams.accountName, shouldRevokeOnApple, nonInteractive);
    ...
    if (credsToClear.pushKey && pushKey) {
      const view = new (_IosPushCredentials().RemoveIosPush)(appLookupParams.accountName, shouldRevokeOnApple, nonInteractive);
