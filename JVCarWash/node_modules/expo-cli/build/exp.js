...
_commander().Command.prototype.prepareCommands = function () {
  return this.commands.filter(function (cmd) {
    ...
    const args = cmd._args.map(humanReadableArgName).join(' '); // Remove alias. We still show this with --help on the command.


    const alias = null; //cmd._alias;

    const description = cmd._description;
    return [cmd._name + (alias ? '|' + alias : '') + ( // Remove the redundant [options] string that's shown after every command.
