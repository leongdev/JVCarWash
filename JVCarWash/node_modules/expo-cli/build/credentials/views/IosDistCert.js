...
class CreateIosDist {
  constructor(accountName, nonInteractive = false) {
...
class RemoveIosDist {
  constructor(accountName, shouldRevoke = false, nonInteractive = false) {
    this.accountName = accountName;
  ...
  async removeSpecific(ctx, selected) {
    ...
    for (const appCredentials of apps) {
      ...
      const view = new (_IosProvisioningProfile().RemoveProvisioningProfile)(this.accountName, shouldRevoke, this.nonInteractive);
...
class CreateOrReuseDistributionCert {
  constructor(app, nonInteractive = false) {
    this.app = app;
    this.nonInteractive = nonInteractive;
  }
  ...
  async open(ctx) {
    ...
    if (existingCertificates.length === 0) {
      const distCert = await new CreateIosDist(this.app.accountName, this.nonInteractive).create(ctx);
