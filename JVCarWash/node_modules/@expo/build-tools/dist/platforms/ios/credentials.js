...
class iOSCredentialsManager {
    ...
    async prepare() {
        const { buildCredentials } = this.job.secrets;
        if (!buildCredentials) {
            throw new Error('credentials are required for an iOS build');
        }
        this.logger.info('Getting distribution certificate fingerprint');
        const certificateFingerprint = distributionCertificateUtils.getFingerprint(buildCredentials.distributionCertificate);
        this.logger.info(`Writing distribution certificate to ${this.distributionCertificatePath}`);
        await fs_extra_1.default.writeFile(this.distributionCertificatePath, Buffer.from(buildCredentials.distributionCertificate.dataBase64, 'base64'));
        try {
            ...
            await this.keychain.importCertificate(this.distributionCertificatePath, buildCredentials.distributionCertificate.password);
            this.logger.info('Initializing provisioning profile');
            this.provisioningProfile = new provisioningProfile_1.default(this.logger, Buffer.from(buildCredentials.provisioningProfileBase64, 'base64'));
